...
function decodeFillOrderReport(base64Data: string, logType: number): any {
  ...
  try {
    ...
    const DECIMALS = 1_000_000_000; // 10^9
 ...
 */
export async function fetchAllTrades(walletAddress: string, limit: number = 50): Promise<DecodedTrade[]> {
...
export function getTradeSummary(trades: DecodedTrade[]) {
  ...
  return {
    ...
    avgQuantity: trades.length > 0 ? trades.reduce((sum, t) => sum + t.quantity, 0) / trades.length : 0,
  };
}
