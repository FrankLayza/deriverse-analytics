      ...
      <div className="relative min-h-screen bg-background">
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/20 backdrop-blur-sm">
          <div className="w-full max-w-md p-8 mx-4 text-center rounded-2xl border border-border bg-card shadow-2xl animate-in fade-in zoom-in-95 duration-300">
            ...
            <p className="text-muted-foreground mb-8">For your security, we disconnected your wallet after 10 minutes of inactivity.</p>
            <div className="flex gap-3 justify-center">
              <Button className="cursor-pointer py-4" variant="outline" onClick={() => setSessionExpired(false)}>Cancel</Button>
              <Button className="cursor-pointer bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => { setSessionExpired(false); setVisible(true); }}>Reconnect Wallet</Button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-background pb-12">
        ...
        <main className="max-w-7xl mx-auto px-6 mt-8">
          <div className="p-6 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-500">
            ...
            <button onClick={sync} className="cursor-pointer mt-4 px-4 py-2 bg-rose-500 text-white rounded-lg text-sm font-bold">Try Again</button>
          </div>
        </main>
      </div>
    );
    ...
    <div className="min-h-screen bg-background">
      <div className="relative z-10">
        ...
        <MotionContainer delay={0.5}>
          <BreakdownCharts fees={data.feeComposition} sessions={data.sessionPerformance} />
        </MotionContainer>
      </div>
    </div>
  );
